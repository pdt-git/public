...
public class SocketSessionTest extends TestCase {
    ...
    protected void setUp() throws Exception {
      ...
      pif.start();
      // This option will make the query evaluation slower, but more verbose
      // concerning non-existing predicates.
	...
	public void testQueryAll() throws Throwable{
		...
		Map map  =s.queryOnce("is_dynamic(member/2).");
	...
	public void test_longAtom() throws Throwable {
		...
		for (int i = 0; i < 60; i++) { // should be: 600000
			sb.append('a');
		}
	...
	public void testException() throws Exception {
	       ...
	       TuPrologPrologInterface tuPinterface = (TuPrologPrologInterface)pif;
	       String warning = pif.getOption(TuPrologPrologInterface.WARNING);
	       pif.setOption(TuPrologPrologInterface.WARNING, "true");
	       final boolean warningShown[] = new boolean[1];
	       warningShown[0] = false;
	       tuPinterface.addWarningListener(new WarningListener() {
...
//					monitor.wait(3000);
			if(!warningShown[0])
		       fail("warning expected");
		    pif.setOption(TuPrologPrologInterface.WARNING, warning);
