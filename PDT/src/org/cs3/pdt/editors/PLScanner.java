...
public class PLScanner extends RuleBasedScanner {
	...
	private void initDynamicPredicates() {
	    if (plDynamicPredicates == null){
	        ...
	        session = PDTPlugin.getDefault().getPrologInterface().getSession();
	            Hashtable solution = session.query("predicate_property(P,dynamic),functor(P,Name,_)");
	...
	private void initKeywords() {
	    if (plKeywords == null){
            ...
            try {
                session = PDTPlugin.getDefault().getPrologInterface().getSession();
                Hashtable solution = session.query("predicate_property(P,built_in),functor(P,Name,_)");
                List keywords = new ArrayList();
                while(solution != null) {
                    keywords.add(solution.get("Name"));
                    solution = session.next();
