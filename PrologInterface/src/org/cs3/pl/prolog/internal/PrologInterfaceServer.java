...
public class PrologInterfaceServer extends MessagingServerLoggingWrapper
	...
	public Hashtable[] queryAll(ClientConnection c, String text){
	    try {
			...
			if (query != null)
				query.close();
			//query = new jpl.Query("catch((" + text + "),error(Formal,Context),fail)"
			//		+ ",flush_output");
	...
	public Hashtable query(ClientConnection c, String text) {
		try {
			...
			if (query != null)
				query.close();
			query = new jpl.Query("catch((" + text + "),A,(format(\"~a ~n\",[A]),fail))"
					+ ",flush_output");
			//query = new jpl.Query( text );
			queries.put(c, query);
	...
	public static void main(String[] args) throws IOException {
	    //ld: this line was recommended by Paul to stop the server from entering interactive debugging whenever it encounters an error.
	    String executable = Prolog.get_default_init_args()[0];
	    Prolog.set_default_init_args(new String[] { executable, "-f", "none", "-g", "set_prolog_flag(debug_on_error,false)", "-q" });

	    File logFile = new File("PrologInterfaceServer.log").getCanonicalFile();
	    if(!logFile.exists()){
	        logFile.getParentFile().mkdirs();
	        logFile.createNewFile();
	    }
	    System.out.println("The server debug output is safed in "+logFile.getAbsolutePath());
	    BufferedOutputStream bufferedOutputStream = new BufferedOutputStream(new FileOutputStream(logFile));
