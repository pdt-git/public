...
showError(Kind, ID,Mesg):-
    ...
    format('~a in method ~a.~a: ~a~nCannot find source location: Probably generated by aspect.~n~n',[Kind,Fqn,Name,Mesg]).
...
createForwMethodExecution(_method, _forwBody,_forwStmts) :- %neu
    ...
    (interfaceT(_class)->
    	(fullQualifiedName(_class,FQN),sformat(Msg,'execution pointcut on interfaces not legal in LAJ: ~a',[FQN]),
    	 throw(Msg));true
    ),	
