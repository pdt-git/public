...
public class PLMarkerUtils {
	...
	public static void addMarkers(IFile file, IProgressMonitor monitor) throws PrologInterfaceException {
		...
		try {
			...
			addMarkersForErrorsAndWarnings(file, session, doc, new SubProgressMonitor(monitor, 1, SubProgressMonitor.PREPEND_MAIN_LABEL_TO_SUBTASK));
			monitor.subTask("Update Prolog Smells Detectors");
			addMarkersForSmellDetectors(file, session, doc, new SubProgressMonitor(monitor, 1, SubProgressMonitor.PREPEND_MAIN_LABEL_TO_SUBTASK));
