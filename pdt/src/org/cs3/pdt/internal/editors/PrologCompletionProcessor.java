...
public class PrologCompletionProcessor implements IContentAssistProcessor {
	...
	private final PLEditor editor;

	public PrologCompletionProcessor(PLEditor editor) {
		this.editor = editor;
	 ...
	 * 
	 * public PrologContextInformation(Predicate data) { this.data = data; }
	 * 
	 * 
	 * (non-Javadoc)
	 * 
	 ...
	 * 
	 * public String getContextDisplayString() { // TODO: when is this
	 * evaluated? // try { // return getHelp(data); // } catch (IOException e) { //
	 * Debug.report(e); // return data.getSignature(); // } return ""; }
	 * 
	 * 
	 * (non-Javadoc)
	 * 
	 ...
	 * 
	 * public Image getImage() { // TODO Auto-generated method stub return null; }
	 * 
	 * 
	 * (non-Javadoc)
	 * 
	 ...
	 * 
	 * public String getInformationDisplayString() { String help; try { help =
	 * getHelp(data); IContextInformation context = null; if (help != null &&
	 * help.length() > 0) return help.substring(data.getLabel().length(), help
	 * .indexOf('\n')); } catch (IOException e) { Debug.report(e); } return ""; } }
	 * 
	 *//*
		 * (non-Javadoc)
		 * 
	...
	private void addPredicateProposals(IDocument document, int begin, int len,
		...
		IMetaInfoProvider prologHelper = plProject.getMetaInfoProvider();
	...
	private List addProposals(int begin, int len, String prefix,
		...
		for (int i = 0; i < elems.length; i++) {
			if (!unique.contains(elems[i].getSignature())) {
				...
				IMetaInfoProvider metaInfoProvider = getMetaInfoProvider();
				ICompletionProposal proposal = new PrologCompletionProposal(
						metaInfoProvider, elems[i], begin, len, prefix);
	...
	private String getHelp(ITextViewer viewer, int documentOffset)
		...
		Goal data = PLEditor.getPrologDataFromOffset(viewer.getDocument(),
				documentOffset);
		if (data == null) {
			return null;
		}
	...
	private IMetaInfoProvider getMetaInfoProvider() {
		...
		IMetaInfoProvider metaInfoProvider = plProject.getMetaInfoProvider();
