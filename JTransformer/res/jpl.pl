...
jpl_pl_lib_version( '3.0.0 alpha').
...
jpl_colour_choose_demo :-
	...
	write( 'you chose '), write( C), nl.

%------------------------------------------------------------------------------

% jpl_date_demo :-
%	packs a new JList into a new JFrame,
%	and populates it with all available Locales in FULL DateFormat

jpl_date_demo :-
	jpl_new( 'javax.swing.JFrame', ['frame with list'], F),
	jpl_new( 'javax.swing.DefaultListModel', [], DLM),
	jpl_call( 'java.util.Calendar', getAvailableLocales, [], Locs),
	jpl_new( 'java.util.Date', [], Date),
	jpl_get( Locs, length, Nlocs),
	jpl_get( 'java.text.DateFormat', 'FULL', Full),
	MaxLoc is Nlocs-1,
	(	between( 0, MaxLoc, I),
		jpl_get( Locs, I, Loc),
		jpl_call( 'java.text.DateFormat', getDateTimeInstance, [Full,Full,Loc], Df),
		jpl_call( Df, format, [Date], S),
		jpl_call( DLM, addElement, [S], _),
		fail
	;	true
	),
...
jpl_text_entry_demo :-
  ...
  %	jpl_call( F, getContentPane, [], CP),
  %	jpl_get( 'java.awt.Color', pink, Pink),
  %	jpl_call( 'javax.swing.JColorChooser', showDialog, [CP,'pick a colo(u)r',Pink], _),

	jpl_call( F, dispose, [], _), 

	write( 'you typed '), write( N), nl.
