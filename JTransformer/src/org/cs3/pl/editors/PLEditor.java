...
import org.cs3.pl.buttons.FindPredicateActionDelegate;
import org.cs3.pl.buttons.ReferencesActionDelegate;
import org.cs3.pl.buttons.SpyPointActionDelegate;
...
public class PLEditor extends TextEditor {
			...
			public void run() {
				...
				try {
					...
					PrologMetaDataManager manager = MetaDataManagerFactory
							.getPrologMetaDataManager(PrologManager
									.getInstance().getHiddenClient(),
									...
									try {
										...
										updateOutline(
												(PLEditor) PDTPlugin
														...
														.getActiveEditor(),
												filename);
										//							PrologDocWriter docwriter = new
										// PrologDocWriter(PDTPlugin.getDefault().getWorkspaceLocation());
	...
	public void createPartControl(final Composite parent) {
		...
		action = new TextEditorAction(JavaEditorMessages.getResourceBundle(), "context assist proposal", this) {
			public void run() {
				assistant.showPossibleCompletions();
			}
		};
        addAction(menuMgr, action, 
        		"Context Assist Proposal", IWorkbenchActionConstants.MB_ADDITIONS,
				ITextEditorActionDefinitionIds.CONTENT_ASSIST_PROPOSALS);
        
        action = new TextEditorAction(JavaEditorMessages.getResourceBundle(), "PLEditor.ContentAssistContextInformation", this) {
			public void run() {
				assistant.showContextInformation();
			}
		};
		addAction(menuMgr, action, 
				"Show Tooltip", IWorkbenchActionConstants.MB_ADDITIONS,
	 ...
	 * @param menuMgr
	 */
	private void addAction(MenuManager menuMgr, Action action, String name, String separator, String id) {
		
		action.setActionDefinitionId(id);
		action.setText(name);
		menuMgr.appendToGroup(separator,action);
		setAction(IJavaEditorActionDefinitionIds.SEARCH_REFERENCES_IN_WORKSPACE,action);
	}
	...
	private MenuManager createPopupMenu() {
		MenuManager menuMgr = new MenuManager("org.cs3.pl.editors.PLEditor","org.cs3.pl.editors.PLEditor"); //$NON-NLS-1$
		menuMgr.addMenuListener(getContextMenuListener());
		menuMgr.add(new Separator(SEP_INSPECT));
		menuMgr.add(new Separator(IWorkbenchActionConstants.MB_ADDITIONS));
		getSourceViewer().getTextWidget().setMenu(menuMgr.createContextMenu(getSourceViewer().getTextWidget()));
	...
	public Object getAdapter(Class required) {
		if (IContentOutlinePage.class.equals(required)) {
			if (fOutlinePage == null) {
				...
				fOutlinePage = new PrologOutline(getDocumentProvider(), this);
	...
	public static PrologElementData getPrologDataFromOffset(IDocument document,
		...
		int endOfWhiteSpace = findEndOfWhiteSpace(document, end, document
				.getLength());
		int arity = 1;
		if (document.getLength() > endOfWhiteSpace && document.getChar(endOfWhiteSpace) == '/') {
		...
		if (document.getLength() == endOfWhiteSpace || document.getChar(endOfWhiteSpace) != '(') {
			if(elementName.endsWith(":"))
				return new PrologElementData(elementName.substring(0,elementName.length()-1),-1);
