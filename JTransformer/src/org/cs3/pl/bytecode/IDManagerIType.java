...
public class IDManagerIType {
	...
	public IDManagerIType(IPrologClient client, FQNTranslator fqn, IIDGenerator ids){
		manager = client;
		this.fqn = fqn;
		this.ids = ids;
	}
...
public static int getArrayDim(String type) {
	...
	private boolean lookupPackage(Package p) {
		Hashtable h = manager.query("packageT(X, '" + p.getName() +"')");
