...
public class NavigatorView extends ViewPart {
	...
	class ViewContentProvider implements IStructuredContentProvider, 
        ...
        private void initializeRoots() throws IOException {
            ...
            roots= new INode[1];
            roots[0]= PEFNode.find(pif,null, "100001");            
        }
		...
		action1 = new Action() {
			public void run() {
				...
				if (dialog.open() != Window.CANCEL) {
					...
					INode node;
					try {
						node = PEFNode.find(PrologManager.getInstance().getHiddenClient(),null,value);
						if(node != null) {
							list.add(node);
							contentProvider.roots = (INode[])list.toArray(new INode[0]);
							viewer.setInput(getViewSite());
							}
					} catch (IOException e) {
						e.printStackTrace();
