...
public class Query implements Enumeration {
	...
	private final Hashtable get2WithNameVars() {
		if (!open) {
			...
			t.getSubst(varnames_to_Terms1,vars_to_Vars1  );
			
			Term error = (Term)varnames_to_Terms1.get("Exception");
			if (!error.isVariable() && showException) {
				Query query = new Query(new Compound("print_message",new Term[]{new Atom("error"),error}));
...
//			Term[] args = goal_.args; // for slight convenience below
			Term[] args = ((Compound)goal_.args[0]).args; // for slight convenience below			
			...
			varnames_to_Terms1 = new Hashtable();
			vars_to_Vars1 = new Hashtable();
