...
%    _c2r = _c2e,
%    _t2  =_t2e,
    % 6) führe auf den expandierten Vorbedingungen und Actionen die Abhängigkeitsanalyse durch
...
effect(replace(_a,_b),_x,_t,_restr,_shared,_l1,_l2) :-
    can_unify(_b, _x),
    % FIX 030319_2: eine positive Abhängigkeit sollte in jedem Fall bestehen,
    % da _b verändert wird und somit die durch _x gebundenen Elemente zu anderen
    % Transformationen führen (im wesentlichen für introduction Aufteilung in
    % interface / code transformer nötig
    % fix wieder entfernt: eine Abhängigkeit soll nur zum execution Advice bestehen (explizit erzeugen)
    not(change_doesnt_matter(_a, _b, _x, _shared)),
...
expandCondition_(_term, empty) :- % lösche elemente die weder abstraktion noch tree sind
%    not(tree(_term)),
%    not(unfoldable(_term)),
    _term \= not(_),
    !.

expandCondition_(not(_abstraction), _ExpandedAbstraction) :- % lösche elemente die weder abstraktion noch tree sind
