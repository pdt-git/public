...
exists(_id):-identT(_id, _, _,_,_), !.
exists(_id):-getFieldT(_id, _, _,_,_,_), !.
exists(_id):-selectT(_id, _, _,_,_,_), !.
exists(_id):-localDefT(_id, _, _,_,_,_), !.
exists(_id):-applyT(_id, _, _,_,_,_,_), !.
exists(_id):-execT(_id, _, _,_), !.
exists(_id):-operationT(_id, _, _,_,_,_), !.
exists(_id):-literalT(_id, _, _,_,_), !.
exists(_id):-blockT(_id, _, _,_), !.
exists(_id):-assignT(_id, _, _,_,_), !.
exists(_id):-conditionalT(_id, _, _,_,_,_), !.
exists(_id):-returnT(_id, _, _,_), !.
exists(_id):-ifT(_id, _, _,_,_,_), !.
exists(_id):-caseT(_id, _, _,_), !.
exists(_id):-indexedT(_id, _, _,_,_), !.
exists(_id):-typeCastT(_id, _, _,_,_), !.
exists(_id):-newClassT(_id, _, _,_,_,_,_,_), !.
exists(_id):-forLoopT(_id, _, _,_,_,_,_), !.
exists(_id):-breakT(_id, _, _,_,_), !.
exists(_id):-importT(_id, _, _), !.
exists(_id):-whileLoopT(_id, _, _,_,_), !.
exists(_id):-newArrayT(_id, _, _,_,_,_), !.
exists(_id):-throwT(_id, _, _,_), !.
exists(_id):-switchT(_id, _, _,_,_), !.
exists(_id):-assignopT(_id, _, _,_,_,_), !.
exists(_id):-catchT(_id, _, _,_,_), !.
exists(_id):-tryT(_id, _, _,_,_,_), !.
exists(_id):-typeTestT(_id, _, _,_,_), !.
exists(_id):-doLoopT(_id, _, _,_,_), !.
exists(_id):-labelT(_id, _, _,_,_), !.
exists(_id):-continueT(_id, _, _,_,_), !.
exists(_id):-synchronizedT(_id, _, _,_,_), !.
exists(_id):-assertT(_id, _, _,_,_), !.
exists(_id):-precedenceT(_id, _, _,_), !.
exists(_id):-nopT(_id, _, _), !.
