...
/**
 */
public class XMLServerStartAndStopStrategy implements
    ...
    public Process startServer(PrologInterface pif) {
        int port = Integer.parseInt(pif.getOption(XMLPrologInterface.PORT));
        String executable = pif.getOption(XMLPrologInterface.EXECUTABLE);
    ...
    public void stopServer(PrologInterface pif, boolean now) {
        try {
            int port = Integer.parseInt(pif
                    .getOption(XMLPrologInterface.PORT));
            if (!Util.probePort(port)) {
                Debug
                        .info("There is no server running, afaics. So i wont stop anything.");
                return;
            }
            XMLClient c = new XMLClient(port);
            
            try {
                c.connect();
                c.writeln("halt(5)");
                c.dispatch_until_next(XMLClient.OK);  
            } catch (Exception e) {
