 ...
 * <u>org.cs3.pl.jtransformer.debug_level</u> which should take one of
...
public class Debug {
	...
	static {
		...
		else if(s.equalsIgnoreCase("WARNING")) debugLevel=LEVEL_WARNING;
		else if(s.equalsIgnoreCase("INFO")) debugLevel=LEVEL_INFO;
		else if(s.equalsIgnoreCase("DEBUG")) debugLevel=LEVEL_DEBUG;
		else {
	...
	/**
	 * sets the level of reporting to the specified constant. Only the LEVEL_* constants
	 * declared in this class are valid argument.
	 * @param level a LEVEL_* constant
	 */
	...
	/**
	 * Sends an warning message to be considered for output. It is printed
	 * if the current debug level is equal or greater to LEVEL_WARNING
	 * 
	 * @param msg the message
	 */
	...
	/**
	 * sends a debug-level Message to be considered for output. It is printed if the
	 * current debug level is equal to LEVEL_DEBUG
	 * 
	 * @param msg the message
	 */
	...
	/**
	 * Sends an error message gto be considered for output. It is printed
	 * if the current debug level is not set to LEVEL_NONE
	 * 
	 * @param msg the message
	 */
	...
	/**
	 * reports an Error or an Exception. Exceptions are treated as Warning-level, while Errors
	 * are considered just that, Errors.
	 *  
	 * @param t a throwable object
	 */
	
	public static void report(Throwable t){
		if(debugLevel==LEVEL_NONE) return ;
	...
	/**
	 * redirects debug output to another PrintStream. The parameter stream may not be null, and must not
	 * be closed by an external class. Use this method to save debug output to a file.
