...
% implements mutable, non-destructive, unique-maps 
...
pdt_map_putall(Map0,Key,Value,Goal,Map):-
	pdt_unique(pdt_map_putall,U),
	nb_setval(U,Map0),
	forall(Goal,
		(	nb_getval(U,MapIn),
			pdt_map_put(MapIn,Key,Value,MapOut),
...
pdt_map_put(In,Key,Value,Out):-
        pdt_rbtree_insert(In,Key,Value,Out).
        
% only succeeds if key is included!
pdt_map_remove(In,Key,Out):-
    pdt_rbtree_delete(In,Key,Out).
