...
term_tokens(From-To,RFrom,RTo,Tokens3):-	   
	...
	Tokens2 = [token(simple,From,To)|Tokens1],
	(	RFrom < From
	->	Tokens3 = [token(gap,RFrom,From)|Tokens2]
	;	Tokens3 = Tokens2		
	).
...
list_tokens(Args,Tail,From,To,TokensOut):-
    ...
    (	Tail == none
    ->	last(Args,LastArg),
    	top_position(LastArg,_,ArgsEnd)
    ;	top_position(Tail,_,ArgsEnd)
    ),
    (	ArgsEnd == To
    ->	TokensTail=[]
    ;	TokensTail=[token(gap,ArgsEnd,To)]
    ),
    TokensOut = [	token(gap,From,ArgFrom),
					token(argument(1),ArgFrom,ArgTo),
					token(argument(2),ArgTo,ArgsEnd)
				|	TokensTail					
				].
...
write_tokens_X([],_,_):-
    !.
write_tokens_X([token(argument(I),From,To)|Tokens],Pos,Indent):-
    !,
    format(atom(SubIndent),"~w~w->",[Indent,I]),    
    sub_position(Pos,I,SubPos),
    term_tokens(SubPos,From,To,SubTokens),
