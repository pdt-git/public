...
Depend-Zyklen, Wait-zyklen, Dead-Locks, bla:

Wait-graph:
A Wartet auf B wenn
i) label(A,B,_,wait) oder
ii) label(A,B,_,build) oder
iii) label(B,A,_,lock) und B obsolet.
...
fp_request_target(Target):-
    thread_self(Me),
    (	building_target(Building),mutable(Target)
    ->	client_block_add_message(Building,depend(Me,Target))	    	
	;	true
	),
    
    client_block_add_message(Target,request(lock(Me,[]))),
    client_block_commit,	
...
fp_run__cleanup(_Target,Catcher):-
    retract('$fp_running'),
    thread_self(Me),
    (	Catcher==fail
    ->	Msg=fail
...
update_target_state(Target,NewState):-
    ...
    log_transition(Target,NewState).
...
arbiter_send_message(fast_lane(ToTarget),FromTarget,Msg):-
	...
	log_send(FromTarget,Msg,ToTarget,SN),
...
client_get_message(Msg):-
    ...
    repeat,
	    	...
	    	call_with_time_limit(2,thread_get_message(builder_msg(SN,_Target,Msg))),
	    	time_limit_exceeded,
	    	fail
	    ),
    !,
...
/*
Wait-graph Eigentlich
A Wartet auf B wenn
i) edge_label(A,B,_,wait) (Ein Thread wartet auf einen anderen)
ii) edge_label(A,B,_,build) Thread wartet soz. auf sich selbst)
iii) edge_label(B,A,_,lock) und B obsolet. 

 

Davon können wir aber abstrahieren, da die wait-pfade der einzelnen Threads keine verzweigungen enthalten.
(EIN thread kann nur auf eine Sache gleichzeitig warten).
Wir sagen  "Ein client wartet bei einem Target auf einen anderen Client." 
Uns interessieren also die "schnittstellen", d.h. Kanten vom Typ i oder iii, im letzteren Fall allerdings nicht alle.
*/

client_waits(Waiting,Target,WaitedFor,wait):- %i-Kanten
    edge_label(_,Target,Waiting,wait),
    edge_label(_,Target,WaitedFor,build).

client_waits(Waiting,Target,WaitedFor,obsolete_lock):- %iii-Kanten
    %Auf ein konsistentes Target wird niemals gewartet. 
    %Auf ein Target das gebaut wird, kann kein Lock existiertn.
    %Daher: existiert ein Lock auf ein target, auf das gewartet wird, so MUSS dieses Lock obsolet sein.
	edge_label(_,Target,Waiting,wait),
	edge_label(Target,_,WaitedFor,lock). 

/* transitive closure of client_waits.
waiting, target,occupier as above.
edges is unified with a list of at(Target,Client) pairs representing the intermediate edges.
*/
client_waits_star(Waiting,Edges,LastTarget,LastOccupier):-
    client_waits(Waiting,Target,Occupier),
    (	Edges=[],
    	LastTarget=Target,
    	LastOccupier=Occupier
    ;	Edges=[at(Target,Occupier)|MoreEdges],
    	client_waits_star(Occupier,MoreEdges,LastTarget,LastOccupier)
    ).
    

/*
Wir nenen ein Target "besetzt", wenn  es gerade nicht verfügbar ist, und auch von einem neu anfragenden Client nicht
neugebaut werden kann. Das ist genau dann der Fall wenn entweder ein obsoletes Lock auf das Target existiert, oder wenn 
das Target bereits von einem anderen Client gebaut wird.
In beiden Fällen gibt es ganau einen Client, der das target "besetzt" hält. Clients die das Target anfragen, müssten auf diesen
"Besetzer" warten.

Beachte: da mehr als ein Client obsolete Locks auf ein Target halten können, kann es mehrere Besetzer eines Targets geben.
Das folgende Prädikat ist aus diesem Grund NICHT deterministisch. 
*/    
target_occupied(Target,Client):-
	\+ available(Target),    
	(	edge_label(_,Target,Client,build)
	->	true
	;	edge_label(_,Target,Client,lock)
	).		


/* 

Cyklensuche muss erfolgen 
 - bei jeder Anfrage,
 - Wenn ein lock obsolet wird.


Die Anfrage eines Targets t durch einen Client c schließt einen Zyklus, wenn
  t durch o besetzt ist und o auf c wartet.
*/
request_closes_cycle(Client,Target):-  
    target_occupied(Target,Occupier),
    client_waits_star(Occupier,_,_,Client),
    !.
/*

Wenn ein Lock eines Clients c auf ein Target t obsolet wird, dann schließt sich hierdurch genau dann ein Zyklus,
wenn c indirekt auf t wartet. Ich bin mir nicht sicher, ob das überhaupt passieren kann. 


Ein Zyklus, der nur aus iii-Kanten besteht, ist unproblematisch, da hier keiner der Clients wartet, d.h. alle Clients sind aktiv.
In jedem anderen Client MUSS es mindestens eine i-Kante geben, denn ein Target kann nicht gebaut werden, solange Locks 
(egal ob obsolet oder nicht) auf dieses Target existieren.

In jedem anderen Zyklus gibt es mindestens eine Typ i-kante. Das heißt es gibt mindestens einen Client, 
der auf eine Nachricht vom Arbiter wartet und somit "ansprechbar" ist.
Eine Anfrage, die gerade vom Arbiter behandelt wird, kann dabei als Typ I-Kante angesehen werden.

 
Wird im zweiten Fall ein KRITISCHER Zyklus, also einer er nicht nur aus iii-kanten besteht, festgestellt, 
dann ist es notwendig der Fall dass der Client, der dieses Lock innehält,
gerade wartet. (folgt aus vorangegangenen beobachtungen und der "Transitivität" von locks.)
In diesem Fall sollte sich dieser Client zurückziehen. (Wie weit?)
*/
