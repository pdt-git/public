...
% Date: 7.9.2004
  ...
  *   ast_node(Id,label,attr1,...).   
      % arg1 is the unique identiy of an AST node of type arg2.
      % The argumetns arg3 to argN are all its single valued attributes.
      % [Attributes are all the values that are not ids of other nodes.]
      % It is assumed that all ast_node facts with the same 
      % value of arg2 have the same attributes.
      
  *   ast_attr(Id,attrName,attrValue). 
      % arg3 is one value of the multi valued attribute arg2 of the 
      % node arg1
  
  *   ast_flag(Id,attrName).           
      % The flag (boolean value) arg2 is true for the node with id arg1.
 
  *   ast_edge(Id1,Relation,Id2).      
      % Represents a relation of type arg3 between the node with 
      % id arg1 and the node with id arg3.
      
  *   ast_edge(Id1,Role1,Id2,Role2).   
