...
ast_reference_type('Tom', abstract_node).
...
ast_node_def('Tom',abstract_node,[
     ast_arg(id,       mult(1,1,no ),  id,    [id]), % <-- convention!!!
     ast_arg(nodeType, mult(1,1,no ),  attr,  [atom])
]).

ast_node_def('Java',abstract_edge,[
     ast_arg(id,         mult(1,1,no ), id,  [id]), % <-- convention!!!
     ast_arg(sourceNode, mult(1,1,no ), id,  [abstract_node]),
     ast_arg(targetNode, mult(1,1,no ), id,  [abstract_node]),
