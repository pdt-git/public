...
public abstract class AbstractSelectionToClipboard implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		...
		try {
			...
			ITextSelection selection = (ITextSelection) editor.getEditorSite()
			...
			AbstractStringBufferWriter writer = getStringBufferWriter();
			CompilationUnit root = parseICompilationUnit(icu);
			VariableIdResolver idResolver = new VariableIdResolver();
			VariableTypeResolver typeResolver = getTypeResolver(idResolver);
			SectionFactGenerator visitor = new SectionFactGenerator(icu, file
					.getFullPath().toString(), idResolver, typeResolver,
					writer, selection);
			root.accept(visitor);
			writer.close();
			if (writer.getLast().length() > 0) {
				...
				String textData = typeResolver.getTypeBindungs()
						+ writer.getLast().substring(0,
								writer.getLast().length() - 2) + ".";

				cb.setContents(new Object[] { textData },
						new Transfer[] { textTransfer });
				MessageDialog.openInformation(shell,
						"Copied corresponding facts to clipboard", textData);
			}
		} catch (Exception e) {
	...
	abstract AbstractStringBufferWriter getStringBufferWriter();
	
	abstract VariableTypeResolver getTypeResolver(VariableIdResolver idResolver);
