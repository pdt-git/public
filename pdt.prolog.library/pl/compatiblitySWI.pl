...
/**
 * close_printf_to_memory(+Key,-Content) is det.
 *
...
stringAppend(S1, _S2, _Ret) :-
	nonvar(S1),
	S1 = unqualified(_A,_B),
	throw(exception).

stringAppend(_S1, S2, _Ret) :-
	nonvar(S2),
	S2 = unqualified(_A,_B),
	throw(exception).

stringAppend(_S1, _S2, Ret) :-
	nonvar(Ret),
	Ret = unqualified(_A,_B),
	throw(exception).

stringAppend(_str1, _str2, Ret) :-
    atom_concat(_str1, _str2, Ret).
