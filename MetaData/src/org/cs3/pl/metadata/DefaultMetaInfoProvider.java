...
public class DefaultMetaInfoProvider implements IMetaInfoProvider {
	...
	public SourceLocation getLocation(String functor, int arity, String contextFile)
		...
		Map solution = session.queryOnce(pdtModulePrefix + "get_file_pos('"
				+ contextFile + "', " + functor + ", " + arity + ",File,Pos,_,_),"
				+
	...
	public String getHelp(PrologElementData data) {
		...
		Map table = null;
		try {
			table = session.query("manual_entry(" + data.getLabel() + ","
					+ data.getArity() + ",Info)");
