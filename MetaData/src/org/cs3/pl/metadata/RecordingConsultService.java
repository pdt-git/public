	...
	private class _OutputStream extends OutputStream {
		...
		public void close() throws IOException {
			...
			fireConsultDataChanged(new ConsultServiceEvent(
					RecordingConsultService.this, unprefixedSymbol));
		}
	...
	protected PrintStream getOutputStream_impl(String s, boolean keepRecord) {
		...
		try {
			...
			if (keepRecord) {
				...
				stream.record = new BufferedOutputStream(new FileOutputStream(
						file));
			}
...
//	}
	...
	private File getFile(String unprefixedSymbol) {
		...
		URI uri = prefix.toURI().resolve(unprefixedSymbol);
		return new File(uri);
	}
	...
	private String getPrefixedSymbol(String s) {
		...
		String symbol = prefixString.endsWith("/") ? prefixString + s : prefixString + "/" + s;
		return symbol;
	}
