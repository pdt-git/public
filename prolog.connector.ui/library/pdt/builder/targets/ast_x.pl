...
generate_ast2(Term, Id,Cx):-
    ...
    ;	top_position(Positions,From,To),
    	pef_property_assert([pef=Id,key=start,value=From]),
    	pef_property_assert([pef=Id,key=end,value=To]),
    	(	functor_position(Positions,FFrom,FTo,FPos)
    	->	pef_property_assert([pef=Id,key=functor_start,value=FFrom]),
    		pef_property_assert([pef=Id,key=functor_end,value=FTo]),
    		pef_property_assert([pef=Id,key=functor_position,value=FPos])
    	;	true
    	)
    ),
    cx_set_gap(Cx,From,Cx1),
	generate_args(1,Arity,Term,Id,Cx1).
...
generate_args(I,N,Term,Id,Cx):-	
	...
	J is I + 1,
	top_position(ArgPositions,_,Gap),
	cx_set_gap(Cx,Gap,NextCx),
