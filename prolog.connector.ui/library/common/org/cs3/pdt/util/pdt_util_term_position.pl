...
top_position(sublist_position(From,To,_,_),From,To).		
...
functor_type(sublist_position(_,_,_,_),list).
...
sub_position(list_position(_,_,[_|[]],none),2,none):-
	!.
sub_position(list_position(_,_,[_|[Elm|Elms]],none),2,sublist_position(From,To,[Elm|Elms],none)):-
    !,
	top_position(Elm,From,_),
    last([Elm|Elms],Last),
    top_position(Last,_,To).
sub_position(list_position(_,_,[_|[]],Tail),2,Tail):-!.
sub_position(list_position(_,_,[_|[Elm|Elms]],Tail),2,sublist_position(From,To,[Elm|Elms],Tail)):-
    !,
	top_position(Elm,From,_),
    top_position(Tail,_,To).
sub_position(sublist_position(From,To,Elms,Tail),I,Pos):-
    !,
	sub_position(list_position(From,To,Elms,Tail),I,Pos).
...
term_tokens(term_position(From,To,FFrom,FTo,Args0),Tokens):-
    inline_functor_position(Args0,FFrom,FTo,Args),
    compound_tokens(Args,From,To,Tokens).
term_tokens(list_position(From,To,Elms,Tail),Tokens):-	   
    list_tokens(list_position(From,To,Elms,Tail),Tokens).
term_tokens(sublist_position(From,To,Elms,Tail),Tokens):-	   
    list_tokens(sublist_position(From,To,Elms,Tail),Tokens).
term_tokens(none,[]).	   
term_tokens(functor_position(From,To),Tokens):-
    term_tokens(From-To,Tokens).
term_tokens(string_position(From,To),Tokens):-
    term_tokens(From-To,Tokens).
term_tokens(brace_term_position(From,To,Arg),Tokens):-
	top_position(Arg,ArgFrom,ArgTo),
	(	ArgTo < To
	->	Tokens0 = [token(argument(1),ArgFrom,ArgTo),token(gap,ArgTo,To)]
	;	Tokens0 = [token(argument(1),ArgFrom,ArgTo)]
	),
	(	From < ArgFrom
	->	Tokens = [token(gap,From,ArgFrom)|Tokens0]
	;	Tokens = Tokens0
	).	
term_tokens(From-To,[token(simple,From,To)]).



% any (sub-)list representation is of the form:
% optional leading gap, head_arg, optional gap and tail_arg, optional trailing gap
list_tokens(Pos,Tokens):-
	top_position(Pos,From,To),    
    sub_position(Pos,1,HeadPos),
    sub_position(Pos,2,TailPos),
    top_position(HeadPos,HeadFrom,HeadTo),
    (	TailPos==none
    ->  TailFrom=To,
    	TailTo=To
	;   top_position(TailPos,TailFrom,TailTo)
	),
    Tokens0=[],
        
    % trailing gap
    (	TailTo < To
    ->	Tokens1=[token(gap,TailTo,To)|Tokens0]
    ;	Tokens1=Tokens0
    ),
    
    %  Tail arg
    (	TailFrom < TailTo
    ->	Tokens2 = [token(argument(2),TailFrom,TailTo)| Tokens1]
    ;	Tokens2 = Tokens1
    ),
    
    % Middle gap
    (	HeadTo < TailFrom
    ->	Tokens3 = [token(gap,HeadTo,TailFrom)| Tokens2]
    ;	Tokens3 = Tokens1
    ),
    
    % head arg
    Tokens4 = [ token(argument(1),HeadFrom,HeadTo)|Tokens3],
    			    		   
	% leading gap
	(	From < HeadFrom
	->	Tokens = [token(gap,From,HeadFrom)|Tokens4]
	;	Tokens = Tokens4
	).    		              


compound_tokens(Args,From,To,Tokens):-    
    compound_tokens_X(Args,1,From,To,Tokens).
    
compound_tokens_X([],_,From,To,Tokens):-
	% Any gap after the last arg belongs to the parent.    
	(	From == To 
	->	Tokens=[]
...
compound_tokens_X([Arg|Args],Pos,From,To,TokensOut):-
    ...
    ->	Tokens2 = [token(functor,ArgFrom,ArgTo)|Tokens],
    	NextPos=Pos
    ;	Tokens2 = [token(argument(Pos),ArgFrom,ArgTo)|Tokens],
    	NextPos is Pos + 1
    ),
    %any gab before the argument belongs to the parent.
    (	From < ArgFrom
    ->	TokensOut = [token(gap,From,ArgFrom)|Tokens2]
    ;	TokensOut = Tokens2
    ),
    NextFrom = ArgTo,
...
write_tokens(Pos):-
    ...
    write_tokens_X(Tokens,Pos,'').

write_tokens_X([],_,_):-
    !.
write_tokens_X([token(argument(I),_From,_To)|Tokens],Pos,Indent):-
    !,
    format(atom(SubIndent),"~w~w->",[Indent,I]),    
    sub_position(Pos,I,SubPos),
    term_tokens(SubPos,SubTokens),
    write_tokens_X(SubTokens,SubPos,SubIndent),
    write_tokens_X(Tokens,Pos,Indent).
write_tokens_X([Token|Tokens],Pos,Indent):-
	format("~w~w~n",[Indent,Token]),
	write_tokens_X(Tokens,Pos,Indent).	   
